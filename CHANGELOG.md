...
- URL pinning (off by default): designate that a specific hostname always opens in a chosen profile. Right-click any page → "Always open this site in…", use the new "Pin this site" affordance in the toolbar popup, or manage rules from the new "Pinned URLs" section in Settings. Matches the full hostname only (`mail.google.com` ≠ `docs.google.com`). When you navigate to a pinned site in the wrong profile, Profilissimo opens it in the right one and closes the source tab. Requires the `webNavigation` permission, which the extension only uses while the toggle is on.
